            ...
            if (args.length == 0) {
                if (!(sender instanceof Player)) {
                    ...
                    if (!(sender.hasPermission("rank.builder")) || !(sender.hasPermission("rank.admin"))) {
                        ...
                        if (!(Armored.build.contains(p))) {
                            Armored.build.add(p);
                            p.sendMessage("Build has been "+ChatColor.GREEN+"enabled"+ChatColor.GRAY+".");
                            return true;
                        } else if (Armored.build.contains(p)) {
                            Armored.build.remove(p);
                ...
                if (!(sender.hasPermission("rank.builder")) || !(sender.hasPermission("rank.admin"))) {
                    ...
                    if (!(Armored.build.contains(target))) {
                        Armored.build.add(target);
                        ...
                        return true;
                    } else if (Armored.build.contains(target)) {
                        Armored.build.remove(target);
